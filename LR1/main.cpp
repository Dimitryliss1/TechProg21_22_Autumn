...
int main() {
    ...
    while(true){
        ...
        if (choice == 0){
            ...
            std::cout << "Pass the absolute or relative path to the file.\nPlease note, if it exists, it'll be overwritten:\n";
            std::string path;
            ...
            if (!out){
                ...
                out << keeper->getStringForFile();
            }
        } else if (choice == 6){
            std::string path;
            std::cout << "Pass absolute or relative path to the file with configuration, generated by program:" << std::endl;
            std::getline (std::cin >> std::ws, path);
            try {
                keeper->readFromFile(path);
            } catch (FormatException&){
                std::cout << "Error occured while reading!\nFix the file or use another one or do everything by hand\n";
            }
        }
    }
    return 0;
